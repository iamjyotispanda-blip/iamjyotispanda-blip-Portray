...
import { ArrowLeft, Save, Ship, Package, TrendingUp, Users, LogOut, Menu, Settings, Building2, Shield, BarChart3, FileText, Home, X, ChevronUp, ChevronDown, PanelLeftClose, PanelLeftOpen } from "lucide-react";
        ...
        <nav className="p-4 space-y-2">
          {navigationItems.map((item) => {
                  ...
                  <div className={`ml-6 space-y-1 ${sidebarCollapsed && !sidebarHovered ? 'hidden' : ''}`}>
                    {item.children.map((child) => {
                      ...
                      return (
                        <div
                          key={child.id}
                          onClick={() => child.id === 'ports' ? setLocation('/ports') : null}
            ...
            return (
              <div
                key={item.id}
                onClick={() => item.id === 'dashboard' ? setLocation('/dashboard') : null}
