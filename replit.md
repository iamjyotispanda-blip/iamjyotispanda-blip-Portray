...
Consistent Layout: All pages use the standard AppLayout template with Portray logo, header notifications, profile logout dropdown, and left toggle to slide menu. All form pages and view pages maintain identical header, left panel, and styling structure. Port form pages match port view page styling exactly, with breadcrumb bar and main content container.
Email Configuration: Email Configuration link should be always available as a top-level navigation item (not nested under Configuration section). Email configuration page should send actual test emails using real SMTP configuration. Port-based email routing system implemented - each port uses its own configured SMTP settings for sending verification and password setup emails. System configuration menu items are now database-driven and grouped under "System Configuration" parent menu in the sidebar navigation, making them visible and manageable through the configuration/menu interface.
Page Management: Comprehensive page management system implemented at /configuration/pages with complete listing of all 33 system pages, categorized by function (Authentication, Dashboard, Organization, Port Management, Terminal Management, Customer/Contract Management, System Configuration). Features include search functionality, category filtering, page status management (active/inactive), deletion capabilities for non-system pages, navigation links, and detailed statistics. System pages are protected from deletion while custom pages can be fully managed.
Role-Based Access: Port Admins redirect to `/port-admin-dashboard` with only "Terminals" navigation link. System Admins have full navigation access to all features. Role-based navigation filtering implemented in AppLayout component. Port Admin dashboard uses AppLayout for consistent navigation structure. Simplified roles page with direct CRUD operations - removed tabs interface and focused on role management with default role templates (System Admin, Port Admin, Terminal Operator, Customer User, Supervisor) available during role creation. Clean interface with search, grid view, and comprehensive role management capabilities.
...
User Verification Workflow: User email verification follows the same pattern as port admin verification - uses GET request for verification, redirects to separate password setup page, and uses existing setup-password component. Port contact verification status progression: "inactive" → "process for verification" (after email verification) → "active" (after password setup completion). This ensures consistency across all verification workflows in the system.
Contact Deletion Cascade: Deleting port admin contacts automatically deactivates associated users. Deleting customer contacts deactivates any user with matching email address. This maintains data integrity and security by ensuring orphaned users cannot access the system.
