...
Role-Based Access: Port Admins redirect to `/port-admin-dashboard` with only "Terminals" navigation link. System Admins have full navigation access to all features. Role-based navigation filtering implemented in AppLayout component. Port Admin dashboard uses AppLayout for consistent navigation structure. Enhanced roles page with dynamic permission management system integrated - includes permission templates (System Admin, Port Admin, Terminal Operator, Customer User, Supervisor), tabbed interface with role management, templates, and statistics views, and template-based role creation with customizable permissions.
