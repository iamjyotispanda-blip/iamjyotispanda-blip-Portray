...
export class MemStorage implements IStorage {
  ...
  async initializeDatabase(): Promise<void> {
    ...
    await this.ensureDefaultOrganizations();
    await this.ensureDefaultMenus();
  }

  private async ensureDefaultUser(): Promise<void> {
    // Check if admin user exists
    const adminUser = await this.getUserByEmail("superadmin@Portray.com");
    if (!adminUser) {
      const hashedPassword = await bcrypt.hash("Csmpl@123", 10);
  ...
  private async ensureDefaultOrganizations(): Promise<void> {
    const organizations = await this.getAllOrganizations();
    if (organizations.length === 0) {
      // Create JSW Infrastructure organization
      const jswOrg = await this.createOrganization({
        organizationName: "JSW Infrastructure Limited",
        displayName: "JSW Infrastructure",
        organizationCode: "JSW-INFRA-001",
        registerOffice: "JSW Centre, Bandra Kurla Complex, Bandra (East), Mumbai - 400051",
  ...
  private async ensureDefaultMenus(): Promise<void> {
    ...
    if (menus.length === 0) {
      ...
      const defaultMenus = [
        ...
        { name: "organizations", label: "Organizations", icon: "Building2", route: "/organizations", sortOrder: 2, menuType: "glink", parentId: null, isActive: true },
        { name: "ports", label: "Ports", icon: "MapPin", route: "/ports", sortOrder: 3, menuType: "glink", parentId: null, isActive: true },
        { name: "customers", label: "Customers", icon: "Users", route: "/customers", sortOrder: 4, menuType: "glink", parentId: null, isActive: true },
        { name: "menu-management", label: "Menu Management", icon: "Menu", route: "/menu-management", sortOrder: 5, menuType: "glink", parentId: null, isActive: true },
      ];
